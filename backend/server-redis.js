...
async function loadGameState(roomCode) {
    try {
        ...
        // Fallback: return null to create new state
    }
    return null;
}
...
async function deleteGameState(roomCode) {
    try {
        ...
        // Continue anyway - room will be cleaned from memory
        return false;
    }
}
...
async function savePlayerConnection(roomCode, playerId, socketId, playerType) {
    try {
        ...
        console.error('Error saving player connection to Redis:', error);
        // Fallback: continue with in-memory tracking only
        return false;
    }
}
...
class GameRoom {
    ...
    startPeriodicSave() {
        ...
        activeIntervals.add(this.saveInterval);
    }
    ...
    removePlayer(playerId) {
        ...
        if (connectedPlayers.length === 0 && this.gameLoop) {
            ...
            activeIntervals.delete(this.gameLoop);
            this.gameLoop = null;
            
            // Clear save interval
            if (this.saveInterval) {
                clearInterval(this.saveInterval);
                activeIntervals.delete(this.saveInterval);
                this.saveInterval = null;
            }
        }
    }
    ...
    startGameLoop() {
        ...
        // Track this interval for cleanup
        activeIntervals.add(this.gameLoop);
    }
...
const globalCleanupInterval = setInterval(async () => {
    let cleanedCount = 0;
    for (const [roomCode, room] of rooms) {
        if (room.getConnectedPlayerCount() === 0) {
            await room.cleanup();
...
activeIntervals.add(globalCleanupInterval);